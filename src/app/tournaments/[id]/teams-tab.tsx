    ...
    <Card>
      <CardHeader className="flex flex-row items-start sm:items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
            {isOrganizer && user && tournament.status === 'open_for_registration' && teams.filter(t => t.isApproved === true).length >= 4 && (
...
function StartTournamentButton({ tournamentId, organizerId }: { tournamentId: string, organizerId: string }) {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            toast({ title: "Success!", description: "Fixtures generated. If the start date is in the future, use Start Immediately in the sidebar to go live." });
        } catch (error: any) {
        ...
        <AlertDialog>
            ...
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        This will generate the initial match schedule and lock registrations. If your play start date is still in the future, the tournament will be Ready to Start until you click Start Immediately.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                    <AlertDialogCancel>Cancel</AlertDialogCancel>
